...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    17 => {
                        log::info!("迁移数据库从 v17 到 v18（MCP/Skills 添加 Kimi Code 支持）");
    ...
    /// v18 -> v19: Claude 会话日志的字节游标列与尾部指纹列。
    ///
    ...
    /// 只有指纹能发现）。
    /// （本 fork 中 v18 已被 Kimi Code 迁移占用，故上游 v18 顺延为 v19。）
    ...
    /// v17 -> v18: Kimi Code harness — add enabled_kimicode to mcp_servers / skills.
...
mod tests {
    ...
    #[test]
    fn migrate_v17_to_v18_adds_kimicode_skill_and_mcp_flags() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            "CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
        ...
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        assert!(Database::has_column(&conn, "mcp_servers", "enabled_kimicode")?);
        assert!(Database::has_column(&conn, "skills", "enabled_kimicode")?);
        let mcp_values: (i64, i64) = conn.query_row(
            "SELECT enabled_codex, enabled_kimicode FROM mcp_servers WHERE id = 'mcp-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        let skill_values: (i64, i64) = conn.query_row(
            "SELECT enabled_codex, enabled_kimicode FROM skills WHERE id = 'skill-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        assert_eq!(mcp_values, (1, 0));
    ...
    #[test]
    fn migrate_v18_to_v19_adds_byte_cursor_to_existing_sync_table() -> Result<(), AppError> {
        // 真实升级路径：v18 库带旧 DDL 的 session_log_sync（无字节游标列，
        // 字节游标曾短暂搭车上游 v17、已执行过 v17 的开发库正是这个形状）
