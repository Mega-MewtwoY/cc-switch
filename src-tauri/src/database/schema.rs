...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（MCP/Skills 添加 Kimi Code 支持）");
    ...
    /// v16 -> v17: Kimi Code harness — add enabled_kimicode to mcp_servers / skills.
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_adds_kimicode_skill_and_mcp_flags() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            "CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
